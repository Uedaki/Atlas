...
Acceleration::Acceleration(std::vector<const Hitable *> &src, const Bound &newBound, std::vector<Acceleration> &pool)
{
	feed(src, newBound, pool);
}

void Acceleration::feed(std::vector<const Hitable *> &src, const Bound &newBound, std::vector<Acceleration> &pool)
{
#ifdef SIMD
	std::array<atlas::Bound, 4> bounds;
#endif
	bound = newBound;
...
void Acceleration::split(std::vector<const Hitable *> &src,
...
bool Acceleration::hit(const atlas::rendering::Ray &ray, const float min, const float max, HitRecord &record) const
{
#ifndef SIMD
...
#else
	atlas::rendering::HitRecord tmpRecord;
	float closest = max;
	bool hasHitAnything = false;
...
void Acceleration::hit(const Cone &ray, const float min, const float max) const
{
	atlas::rendering::HitRecord tmpRecord;
	float closest = max;
	bool hasHitAnything = false;

	if (bounds[0].getMin() != bounds[0].getMax())
	{
		gte::TIQuery query;
		auto res = query(bounds[0], ray.bound);
		if (res.intersect)
		{
			elements[0]->hit(ray, 0.01f, closest);
		}
	}

	if (bounds[1].getMin() != bounds[1].getMax())
	{
		gte::TIQuery query;
		auto res = query(bounds[1], ray.bound);
		if (res.intersect)
		{
			elements[1]->hit(ray, 0.01f, closest);
		}
	}

	if (bounds[2].getMin() != bounds[2].getMax())
	{
		gte::TIQuery query;
		auto res = query(bounds[2], ray.bound);
		if (res.intersect)
		{
			elements[2]->hit(ray, 0.01f, closest);
		}
	}

	if (bounds[3].getMin() != bounds[3].getMax())
	{
		gte::TIQuery query;
		auto res = query(bounds[3], ray.bound);
		if (res.intersect)
		{
			elements[3]->hit(ray, 0.01f, closest);
		}
	}
}

#ifdef SIMD
bool4 Acceleration::simdIntersect(const atlas::rendering::Ray &ray, const float min, const float max) const
{
	float4 tmin;
	float4 tmax;
	float4 tymin;
