...
namespace gte
{
    ...
    public:
        ...
        Result operator()(Bound const &box, BoundingCone const &cone)
        {
            Result result;
    ...
    protected:
        ...
        std::array<glm::vec3, MAX_VERTICES> mVertices;
        ...
        static void ComputeBoxHeightInterval(Bound const &box, BoundingCone const &cone,
            float &boxMinHeight, float &boxMaxHeight)
        {
            glm::vec3 C, e;
            C = (box.getMax() + box.getMin()) * 0.5f;
            e = (box.getMax() - box.getMin()) * 0.5f;
           // box.GetCenteredForm(C, e);
            glm::vec3 const &V = cone.origin;
            glm::vec3 const &U = cone.dir;
            glm::vec3 CmV = C - V;
            float DdCmV = glm::dot(U, CmV);
            float radius = e[0] * std::abs(U[0]) + e[1] * std::abs(U[1]) + e[2] * std::abs(U[2]);
            boxMinHeight = DdCmV - radius;
            boxMaxHeight = DdCmV + radius;
        }

        static bool ConeAxisIntersectsBox(Bound const &box, BoundingCone const &cone)
        {
            //if (cone.IsFinite())
            //{
            ...
            //else
            {
                atlas::rendering::Ray ray(cone.origin, cone.dir);
                return (box.intersect(ray, cone.tmin, cone.tmax));

                //Ray<3, Real> ray;
                //ray.origin = cone.origin + cone.tmin * cone.dir;
                //ray.direction = cone.dir;
                //auto rbResult = TIQuery()(ray, box);
        ...
        bool BoxFullyInConeSlab(Bound const &box, float boxMinHeight, float boxMaxHeight, BoundingCone const &cone)
        {
            // Compute the box vertices relative to cone vertex as origin.
            mVertices[0] = { box.getMin()[0], box.getMin()[1], box.getMin()[2] };
            mVertices[1] = { box.getMax()[0], box.getMin()[1], box.getMin()[2] };
            mVertices[2] = { box.getMin()[0], box.getMax()[1], box.getMin()[2] };
            mVertices[3] = { box.getMax()[0], box.getMax()[1], box.getMin()[2] };
            mVertices[4] = { box.getMin()[0], box.getMin()[1], box.getMax()[2] };
            mVertices[5] = { box.getMax()[0], box.getMin()[1], box.getMax()[2] };
            mVertices[6] = { box.getMin()[0], box.getMax()[1], box.getMax()[2] };
            mVertices[7] = { box.getMax()[0], box.getMax()[1], box.getMax()[2] };
            ...
            float coneMaxHeight = cone.tmax;
            //float coneMaxHeight = (cone.IsFinite() ? cone.GetMaxHeight() : std::numeric_limits<float>::max());
        ...
        static bool HasPointInsideCone(glm::vec3 const &P0, glm::vec3 const &P1,
            BoundingCone const &cone)
        {
            ...
            float g = glm::dot(U, P0) - cone.dot * P0.length();
            if (g > (float)0)
            {
            ...
            g = glm::dot(U, P1) - cone.dot * P1.length();
            if (g > (float)0)
            {
            ...
            glm::vec3 E = P1 - P0;
            glm::vec3 crossP0U = glm::cross(P0, U);
            glm::vec3 crossP0E = glm::cross(P0, E);
            float dphi0 = glm::dot(crossP0E, crossP0U);
            if (dphi0 > (float)0)
            {
                glm::vec3 crossP1U = glm::cross(P1, U);
                float dphi1 = glm::dot(crossP0E, crossP1U);
                if (dphi1 < (float)0)
                {
                    float t = dphi0 / (dphi0 - dphi1);
                    glm::vec3 PMax = P0 + t * E;
                    g = glm::dot(U, PMax) - cone.dot * PMax.length();
                    if (g > (float)0)
                    {
        ...
        bool CandidatesHavePointInsideCone(BoundingCone const &cone) const
        {
            for (size_t i = 0; i < mNumCandidateEdges; ++i)
            {
                ...
                glm::vec3 const &P0 = mVertices[edge[0]];
                glm::vec3 const &P1 = mVertices[edge[1]];
        ...
        void ComputeCandidatesOnBoxEdges(BoundingCone const &cone)
        {
            ...
            {
                float h = glm::dot(cone.dir, mVertices[i]);
                float coneMaxHeight = cone.tmax;
                //float coneMaxHeight = (cone.IsFinite() ? cone.GetMaxHeight() : std::numeric_limits<float>::max());
