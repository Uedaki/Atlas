...
Lambert::Lambert(const atlas::MaterialInput &a)
	: albedo(a)
{}
...
bool Lambert::scatter(const atlas::rendering::Ray &in, const atlas::rendering::HitRecord &hit, glm::vec3 &attenuation, atlas::rendering::Ray &scattered) const
{
	...
	glm::vec3 dir = hit.normal + atlas::Tools::randomUniformHemisphere();
	scattered = atlas::rendering::Ray(hit.p, dir);
	attenuation = albedo.sample(hit);
	return (true);
}

Metal::Metal(const atlas::MaterialInput &a, const float f)
	: albedo(a), fuzz(f)
{}
...
Light::Light(atlas::MaterialInput &a)
	: emit(a)
{}

bool Light::scatter(const atlas::rendering::Ray &in, const atlas::rendering::HitRecord &hit, glm::vec3 &attenuation, atlas::rendering::Ray &scattered) const
{
	return (false);
}

glm::vec3 Light::emitted(const atlas::rendering::HitRecord &hit) const
{
