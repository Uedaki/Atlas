...
bool atlas::getMediumScatteringProperties(const std::string &name, Spectrum *sigma_a, Spectrum *sigma_prime_s)
{
    for (MeasuredSS &mss : SubsurfaceParameterTable)
    {
        if (name == mss.name) {
            *sigma_a = Spectrum(mss.sigma_a[0], mss.sigma_a[1], mss.sigma_a[2]);
            *sigma_prime_s = Spectrum(mss.sigma_prime_s[0], mss.sigma_prime_s[1], mss.sigma_prime_s[2]);
            return true;
        }
    }
    return false;
