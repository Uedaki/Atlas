...
atlas::Bound::Bound(const glm::vec3 &min, const glm::vec3 &max)
	: min(min), max(max)
{}

void atlas::Bound::operator=(const Bound &bound)
{
	max = bound.max;
	min = bound.min;
}
...
int atlas::Bound::largestAxis() const
{
...
glm::vec3 atlas::Bound::getCenter() const
{
	return (min + (max - min) * 0.5f);
}

bool atlas::Bound::intersect(const rendering::Ray &ray, float distMin, float distMax) const
{
	float tmin;
	float tmax;
	float tymin;
...
bool4 atlas::Bound::simdIntersect(SimdRay &ray, float4 distMin, float4 distMax) const
{
	float4 tmin;
	float4 tmax;
	float4 tymin;
