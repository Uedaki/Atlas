...
#define SHUFFLE4(V, X, Y, Z, W) S4Float(_mm_shuffle_ps((V).m, (V).m, _MM_SHUFFLE(W, Z, Y, X)))
	...
	SIMD_INLINE S4Bool operator!(S4Bool &b)
	{
		return (b == S4Bool(0, 0, 0, 0));
	}

	SIMD_INLINE void swap(S4Float &v1, S4Float &v2, const S4Bool &mask)
	{
		S4Float tmp = v1;
	...
	SIMD_INLINE S4Float clamp(S4Float v, S4Float min, S4Float max)
	{
		S4Bool minMask = v < min;
		S4Bool maxMask = v > max;

		S4Float r = v;
		if (any(minMask))
		{
			r = select(v, min, minMask);
		}

		if (any(maxMask))
		{
			r = select(v, max, maxMask);
		}
		return (r);
	}
