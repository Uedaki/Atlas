...
#include "../Acheron/BinFile.h"

int main()
{
	//std::cout << "Compact ray size " << sizeof(CompactRay) << std::endl;

	//glm::vec3 c = CompactRay::toColor(CompactRay::toRgb9e5(glm::vec3(1, 0.5, 0.8)));
	//std::cout << "(1, 0.5, 0.8) -> " << c.r << " " << c.g << " " << c.b << std::endl;

	//glm::vec3 n = glm::normalize(glm::vec3(4, 2, 18));
	//glm::vec3 res = CompactRay::octDecode(CompactRay::octEncode(n));
	//std::cout << "dir " << n.x << " " << n.y << " " << n.z << " -> " << res.x << " " << res.y << " " << res.z << std::endl;
